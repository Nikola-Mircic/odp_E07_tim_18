...
export class UserController {
  ...
  private async getUserById(req: any, res: any): Promise<void> {
    try {
      ...
      if (isNaN(id)) {
        res.status(StatusCodes.BAD_REQUEST).json({ success: false, message: 'Invalid user ID' });
        return;
      }
  ...
  private async getUserByEmail(req: any, res: any): Promise<void> {
    try {
      ...
      if (!email || !emailValidator(email)) {
        res.status(StatusCodes.BAD_REQUEST).json({ success: false, message: 'Invalid email' });
        return;
      }
